...
const registerUser = async (req, res) => {
  const { gender, username, firstName, lastName, email, password, role ,certification } = req.body;
  ...
  try {
    ...
    const blocked = role === 'Professionnal' || role === 'initiator';
...
const authUser = async (req, res) => {
  ...
  try {
    ...
    if (user.isDeleted) {
      return res.status(410).json({ message: "User deleted" });
    }
    return res.json({
      _id: user._id,
      username: user.username,
